...
export const getTimeFrameRange = (timeFrame: string): TimeFrameRange => {
  const now = new Date();
...
export const getPreviousTimeFrameRange = (timeFrame: string): TimeFrameRange => {
  const now = new Date();
  const start = new Date(now);
  start.setHours(0, 0, 0, 0);
...
export const calculateData = (transactions: Transaction[]): TransactionSummary => {
...
export const generateChartPoints = (timeFrame: string): ChartPoint[] => {
  const now = new Date();
